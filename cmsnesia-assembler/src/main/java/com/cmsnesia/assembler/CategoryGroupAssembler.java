...
public class CategoryGroupAssembler extends Assembler<CategoryGroup, CategoryGroupDto> {
    ...
    public CategoryGroup fromDto(@Nonnull CategoryGroupDto dto) {
        CategoryGroup category = CategoryGroup.builder().id(dto.getId()).name(dto.getName()).categoryIds(dto.getCategoryIds() == null ? new HashSet<>() : dto.getCategoryIds()).build();
        return category;
    }
    ...
    public CategoryGroupDto fromEntity(@Nonnull CategoryGroup entity) {
        return CategoryGroupDto.builder().id(entity.getId()).name(entity.getName()).categoryIds(entity.getCategoryIds() == null ? new HashSet<>() : entity.getCategoryIds()).build();
    }
