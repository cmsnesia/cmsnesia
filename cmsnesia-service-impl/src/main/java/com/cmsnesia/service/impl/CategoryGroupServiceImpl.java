...
public class CategoryGroupServiceImpl implements CategoryGroupService {
    ...
    public Mono<Result<CategoryGroupDto>> add(AuthDto session, CategoryGroupDto dto) {
        return categoryGroupRepo.exists(session, null, dto.getName())
                .flatMap(exists -> {
                    if (!exists) {
    ...
    public Mono<Result<CategoryGroupDto>> edit(AuthDto session, CategoryGroupDto dto) {
        return categoryGroupRepo.exists(session, dto.getId(), dto.getName())
                .flatMap(exists -> {
                    if (exists) {
                        return categoryGroupRepo.findById(dto.getId())
    ...
    public Mono<Result<CategoryGroupDto>> delete(AuthDto session, CategoryGroupDto dto) {
        return categoryGroupRepo.exists(session, dto.getId(), dto.getName())
                .flatMap(exists -> {
                    if (exists) {
                        return categoryGroupRepo.deleteById(dto.getId())
    ...
    @Override
    public Mono<Page<CategoryGroupDto>> find(AuthDto session, CategoryGroupDto dto, Pageable pageable) {
        return categoryGroupRepo
                        ...
                        count -> {
                            Mono<List<CategoryGroupDto>> mono =
                                    categoryGroupRepo
                                            ...
                                            .collectList();
                            return mono.map(
                                    categoryGroupDtos -> new PageImpl<CategoryGroupDto>(categoryGroupDtos, pageable, count));
                        });
    }
