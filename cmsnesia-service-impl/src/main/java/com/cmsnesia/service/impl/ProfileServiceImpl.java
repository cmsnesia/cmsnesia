...
public class ProfileServiceImpl implements ProfileService {
    ...
    public Mono<Result<ProfileDto>> add(AuthDto session, ProfileDto dto) {
        return profileRepo.exists(session)
                .flatMap(exists -> {
                    if (exists) {
                    ...
                    return profileRepo.save(profile)
                            .map(saved -> Result.build(profileAssembler.fromEntity(saved), StatusCode.SAVE_SUCCESS));
                });
    }
    ...
    public Mono<Result<ProfileDto>> find(AuthDto session, IdRequest idRequest) {
        return profileRepo.find(session, idRequest)
