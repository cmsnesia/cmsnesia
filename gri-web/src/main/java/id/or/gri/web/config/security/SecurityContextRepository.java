package id.or.gri.web.config.security;
...
public class SecurityContextRepository implements ServerSecurityContextRepository {
    ...
    public Mono<SecurityContext> load(ServerWebExchange exchange) {
        ...
        if (authHeader != null && authHeader.startsWith("Bearer ")) {
            ...
            return authDtoMono.map(authDto -> {
                Set<GrantedAuthority> authorities = authDto.getRoles().stream()
                        .map(role -> new SimpleGrantedAuthority(role))
                        .collect(Collectors.toSet());
                Authentication authentication = new UsernamePasswordAuthenticationToken(authDto, null, authorities);
