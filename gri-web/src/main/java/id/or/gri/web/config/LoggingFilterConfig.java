package id.or.gri.web.config;

import id.or.gri.web.config.interceptor.RequestLoggingInterceptor;
import id.or.gri.web.config.interceptor.ResponseLoggingInterceptor;
...
public class LoggingFilterConfig implements WebFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
        if (ignorePatterns != null && exchange.getRequest().getURI().getPath().matches(ignorePatterns)) {
            ...
            if (useContentLength && (header == null || header.get(0).equals("0"))) {
                if (logHeaders)
                    log.info("Request: method={}, uri={}, headers={}", exchange.getRequest().getMethod(),
                            exchange.getRequest().getURI().getPath(), exchange.getRequest().getHeaders());
                else
                    log.info("Request: method={}, uri={}", exchange.getRequest().getMethod(),
    ...
    private void logResponse(long startTime, ServerHttpResponse response, int overriddenStatus) {
        ...
        if (useContentLength && (header == null || header.get(0).equals("0"))) {
            if (logHeaders) {
                log.info("Response({} ms): status={}, headers={}", "X-Response-Time: " + duration,
                        "X-Response-Status: " + overriddenStatus, response.getHeaders());
            } else {
                log.info("Response({} ms): status={}", "X-Response-Time: " + duration,
